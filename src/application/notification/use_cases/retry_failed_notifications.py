"""Retry Failed Notifications Use Case."""
...
class RetryFailedNotificationsUseCase:
    ...
    def __init__(
        ...
        Args:
            ...
            batch_size: Number of notifications to retry at once
        """
    ...
    async def execute(self) -> List[NotificationDTO]:
        ...
        Returns:
            List of retried notification DTOs
        """
        # Get failed retryable notifications
        notifications = await self._notification_repository.find_failed_retryable(
        ...
        logger.info(f"Retrying {len(notifications)} failed notifications")
        ...
        for notification in notifications:
            try:
                ...
                await self._notification_repository.save(notification)
                retried.append(self._to_dto(notification))

            except Exception as e:
                logger.error(
                    f"Error retrying notification {notification.notification_id.value}: {e}",
                    exc_info=True,
                )
                try:
