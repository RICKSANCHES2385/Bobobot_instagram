"""Fetch Instagram Posts Use Case."""
...
class FetchInstagramPostsCommand:
    """Command for fetching Instagram posts."""
...
class FetchInstagramPostsUseCase(UseCase[FetchInstagramPostsCommand, List[InstagramPostDTO]]):
    """Use case for fetching Instagram posts."""
    
    def __init__(self, instagram_api_client):
        self.instagram_api_client = instagram_api_client
    
    async def execute(self, command: FetchInstagramPostsCommand) -> List[InstagramPostDTO]:
        """Execute the use case."""
        medias, next_cursor = await self.instagram_api_client.get_user_medias_chunk(
            command.user_id, 
            command.cursor
        )
                ...
                if media_type == "CAROUSEL":
                    ...
                    for item in carousel_media:
                        if item.get("media_type") == 2:
                            url = item.get("video_versions", [{}])[0].get("url")
                        else:
